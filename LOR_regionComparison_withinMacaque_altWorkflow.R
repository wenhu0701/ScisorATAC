...
stateFile_case <- fread(file.path(dataDir,'PFC_vs_VIS_Macaque/multiVeloMatrixFiles/Macaque_2PFC_multivelo_dPSItestedGenes_Cell_States_mtx.csv'))
stateFile_ctrl <- fread(file.path(dataDir,'PFC_vs_VIS_Macaque/multiVeloMatrixFiles/Macaque_2VIS_multivelo_dPSItestedGenes_Cell_States_mtx.csv'))
colnames(stateFile_case)[1] <- colnames(stateFile_ctrl)[1] <- "Sample_BC"

print("Reading in all info")
allInfo_case <- fread(file.path(dataDir,'PFC_vs_VIS_Macaque/AllInfoFiles/AllInfo_PFC.gz'))
allInfo_case <- allInfo_case[,c(1:5,9)]

allInfo_ctrl <- fread(file.path(dataDir,'PFC_vs_VIS_Macaque/AllInfoFiles/AllInfo_VIS.gz'))
...
gene2clear <- read.table(file.path(dataDir,'bidirectional_humanMacaque'), header = F)[c(1,2)] %>% distinct()
gene2clear <- gene2clear %>% separate(V1,into=c("ENS","Suffix"),sep = "[.]") %>% select(-Suffix)
colnames(gene2clear) <- c("ENS","Clear")

exonList <- read.table(file.path(dataDir,'PFC_vs_VIS_Macaque/testedExons_PFCvVIS'), header = F)
colnames(exonList) <- c("Exon","ENS","Clear")
...
fullMat <- full_join(caseRes,controlRes, by = c("CT","Exon","stateID","GeneName","GeneID")) %>% 
    mutate(statePSI.x = ifelse(is.na(statePSI.x), -1, statePSI.x),
           Sample.x = ifelse(is.na(Sample.x), "PFC", Sample.x),
           statePSI.y = ifelse(is.na(statePSI.y), -1, statePSI.y),
          Sample.y = ifelse(is.na(Sample.y), "VIS", Sample.y),)
head(fullMat,5)
                                        
                                        
dir.create('../2024_02_29_multiVelo/output/v2_macaque/')
write.table(fullMat,'../2024_02_29_multiVelo/output/v2_macaque/PFC_vs_VIS_fullMat_altWorkflow.tab',sep = "\t",
